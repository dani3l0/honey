...
export const getConfigs = async () => {
	let respMain = await fetch(isDev ? "http://localhost:4208/api/config" : "/api/config")
	if (respMain.ok) configMain.set((await respMain.json()).data)

	let respSystem = await fetch(isDev ? "http://localhost:4208/api/admin/getSystem" : "/api/admin/getSystem", {
		credentials: "include"
	})
